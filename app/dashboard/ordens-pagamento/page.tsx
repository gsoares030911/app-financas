...
export default async function OrdensPagamentoPage() {
  ...
  const [{ data: orders }, { data: producers }, cnabConfig] = await Promise.all([
    ...
    supabase
      .from('producers')
      .select('id, full_name, email, phone, cpf_cnpj, bank_name, bank_agency, bank_account, pix_key')
      .order('full_name'),
    getCnabConfig(),
  ])
  ...
  return (
    <div className="space-y-6">
      ...
      <OrdensListClient
        ...
        producers={(producers ?? []) as Pick<Producer, 'id' | 'full_name' | 'email' | 'phone' | 'cpf_cnpj' | 'bank_name' | 'bank_agency' | 'bank_account' | 'pix_key'>[]}
